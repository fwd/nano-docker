...
if [[ "$1" == "-t" ]] || [[ "$2" == "-t" ]] || [[ "$3" == "-t" ]] || [[ "$4" == "-t" ]] || [[ "$5" == "-v" ]] || [[ "$6" == "-v" ]] || [[ "$7" == "-v" ]]; then
    if [[ "$2" == "-t" ]] || [[ "$2" == "-v" ]]; then tag=$3; fi;
    if [[ "$3" == "-t" ]] || [[ "$3" == "-v" ]]; then tag=$4; fi;
    if [[ "$4" == "-t" ]] || [[ "$4" == "-v" ]]; then tag=$5; fi;
    if [[ "$5" == "-t" ]] || [[ "$5" == "-v" ]]; then tag=$6; fi;
    if [[ "$7" == "-t" ]] || [[ "$7" == "-v" ]]; then tag=$8; fi;
fi

if [[ "$1" == "-p" ]] || [[ "$2" == "-p" ]] || [[ "$3" == "-p" ]] || [[ "$4" == "-p" ]] || [[ "$5" == "-p" ]] || [[ "$6" == "-p" ]] || [[ "$7" == "-p" ]]; then
    if [[ "$2" == "-p" ]] || [[ "$2" == "-port" ]]; then port=$3; fi;
    if [[ "$3" == "-p" ]] || [[ "$3" == "-port" ]]; then port=$4; fi;
    if [[ "$4" == "-p" ]] || [[ "$4" == "-port" ]]; then port=$5; fi;
    if [[ "$5" == "-p" ]] || [[ "$5" == "-port" ]]; then port=$6; fi;
    if [[ "$6" == "-p" ]] || [[ "$6" == "-port" ]]; then port=$7; fi;
    if [[ "$7" == "-p" ]] || [[ "$7" == "-port" ]]; then port=$8; fi;
    if [[ "$8" == "-p" ]] || [[ "$8" == "-port" ]]; then port=$9; fi;
fi
